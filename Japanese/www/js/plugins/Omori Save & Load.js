...
Window_OmoriFileCommand.prototype.makeCommandList = function () {
  this.addCommand("セーブ", 'save', this._canSave);
  this.addCommand("ロード", 'load', this._canLoad);
};
...
BattleManager.getActorInputOrder = function() {
    let churi = $gameParty.members();
    let toring = churi.map( (el, index) => [index, el.agi, el.isAlive() && el.isBattleMember()] )
	
    toring.sort( (a, b) => b[1] > a[1] )
    toring = toring.filter(_ => _[2] )
	
    return toring.map(_ => _[0] )
};