...
Window_OmoriFileInformation.prototype.refresh = function() {
  ...
  this.contents.drawText(LanguageManager.getMessageData("XX_BLUE.Omori_Save_Load").file.format(id), 10 + 30, 2, 100, this.contents.fontSize); //10 + 30, -5, 100
  // If Valid
  if (valid) {
...
Window_OmoriFileCommand.prototype.makeCommandList = function () {
  this.addCommand("저장", 'save', this._canSave);
  this.addCommand("불러오기", 'load', this._canLoad);
};
...
Window_OmoriFilePrompt.prototype.makeCommandList = function () {
  this.addCommand("예", 'ok');
  this.addCommand("아니오", 'cancel');
};
//=============================================================================
// * Refresh
//=============================================================================
...
BattleManager.getActorInputOrder = function() {
    let actorsc = $gameParty.members();
    let listsc = actorsc.map( (el, index) => [index, el.agi, el.isAlive() && el.isBattleMember()] )
	
    listsc.sort( (a, b) => b[1] > a[1] )
    listsc = listsc.filter(_ => _[2] )
	
    return listsc.map(_ => _[0] )
};